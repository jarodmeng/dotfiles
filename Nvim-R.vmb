...
Command
  ...
  . Knit and ODT (cur file) (only .Rmd)                \ko
...
Vim can jump to functions defined in other files if you press CTRL-] over
...
the R function `rtags()` to build the tags file for the R scripts in the
current directory. Please read |Nvim-R-tagsfile| to learn how to create a tags
file referencing source code located in other directories, including the
...
------------------------------------------------------------------------------
5.5. R must be started by Vim/Neovim~
...
Nvim-R to: perform omnicompletion, show R documentation in an Vim buffer,
and update the Object Browser. The two last ones are used by Nvim-R and by
nvimcom to know that the connections are valid. Unless you are running R in a
Neovim's built-in terminal (which is the default on Unix), if you use Vim
to start R, but then close Vim, some variables will become outdated.
Additionally, Nvim-R sets the value of its internal variable SendCmdToR from
SendCmdToR_fake to the appropriate value when R is successfully started. It is
possible to set the values of all those variables manually, but, as you can
see below, it is not practical to do so. If you have either started R before
Vim or closed Vim and opened it again and really want full
communication between Vim and R, you can try the following (not all
...
>
   ...
   library(nvimcom)
<
If you are running R in a Tmux session (see |Nvim-R-Tmux|) Vim will also
need to know the name of Tmux session and Tmux pane where R is running.
...
------------------------------------------------------------------------------
							      ...
							      *R_save_win_pos*
							   *R_arrange_windows*
6.3. Time for the paste command on Windows~
...
If you do not want to see R documentation in an Vim buffer, put in your
|vimrc|:
>
   let R_nvimpager = "no"
<
...
way it works, please, read the subsection 6.5.2 below.
...
------------------------------------------------------------------------------
								      *R_path*
								      *R_app*
6.7. R path and application name~
...
the tags file. Otherwise, you can use the nvimcom function `etags2ctags()` to
convert the Emacs tags file into an Vim one. To jump to a function
...
<
The Object Browser will be created in an independent Vim instance in a Tmux
pane. If you prefer the Object Browser always created as an Vim split
window, put in your |vimrc|:
>
   let R_tmux_ob = 0
<
